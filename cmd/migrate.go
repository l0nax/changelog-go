...
func migrateOldChangeEntry(path string) error {
	...
	for scanner.Scan() {
		...
		case strings.HasPrefix(line, "type: "):
			...
			case "0":
				entry.ChangeTypeID = "new_feat"
			case "1":
				entry.ChangeTypeID = "bug_fix"
			case "2":
				entry.ChangeTypeID = "feat_change"
			case "3":
				entry.ChangeTypeID = "deprecate"
			case "4":
				entry.ChangeTypeID = "rem_feat"
			case "5":
				entry.ChangeTypeID = "security"
			case "6":
				entry.ChangeTypeID = "other"
			default:
